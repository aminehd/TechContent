...
function loadRun(idx) {
  ...
  // Skip leading micro-snaps that have no viz data yet
  const firstMeaningful = snapshots.findIndex(s => Object.keys(s.data).length > 0)
  showSnap(firstMeaningful > 0 ? firstMeaningful : 0)
}
...
fetchFrames("lc200")
  .then(data => {
    document.getElementById("prob-title").textContent = `${data.problem} · ${data.title}`
    runs = data.runs
